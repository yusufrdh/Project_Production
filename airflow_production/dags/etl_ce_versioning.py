...
def proses_etl_ce():
    # --- [ADVANCE] ARAHKAN KE FOLDER ISOLASI CE ---
    folder_path = '/opt/airflow/data_kpc/CE'
    ...
    if not all_files:
        print("⚠️ Tidak ada file Excel di folder CE.")
        return

    hook = MsSqlHook(mssql_conn_id=CONN_ID)
    engine = hook.get_sqlalchemy_engine()

    # Reset Data Actual CE
    print("🧹 Truncate Table CE Actual...")
    hook.run("TRUNCATE TABLE dbo.ce_actual")
    
    # hook.run("TRUNCATE TABLE dbo.ce_plan") # Optional Testing
    
    pit_map = get_pit_map()

    for nama_file in all_files:
        full_path = f'{folder_path}/{nama_file}'
        print(f"\n📄 PROSES FILE CE: {nama_file}")
        ...
        for sheet_name in xls.sheet_names:
            ...
            # Kalau ada sheet "CM" atau "OB" di folder CE, TENDANG!
            if re.match(r'^(CM|OB|CP)\s*', sheet_name, re.IGNORECASE):
                continue

            # --- [LOGIC 2] CLEANING NAMA SHEET ---
            # Hapus prefix "CE" atau "Coal Exposed"
            # Regex ini menangkap "CE Bendili" atau "Coal Exposed Bendili"
            clean_name = re.sub(r'^(CE|Coal Exposed)\s*', '', sheet_name, flags=re.IGNORECASE).strip()
        ...
        if not df_actual_total.empty:
            ...
            # Target Tabel CE Actual
            df_actual_total[cols].to_sql('ce_actual', con=engine, if_exists='append', index=False, chunksize=1000)
            print(f"   ✅ Inserted {len(df_actual_total)} rows to ce_actual.")

        if not df_plan_excel.empty:
            proses_versioning_ce(df_plan_excel, hook, engine)

    print("\n✅ SEMUA FILE CE SELESAI!")

def proses_versioning_ce(df_new, hook, engine):
    print("   🔍 Checking Versioning for Plan CE...")
    # TARGET: dbo.ce_plan
    sql_existing = "SELECT id_pit, plan_date, plan_qty as db_qty, version as db_ver FROM dbo.ce_plan WHERE is_active = 1"
    ...
    if not new_records.empty:
        ...
        new_records[cols].to_sql('ce_plan', con=engine, if_exists='append', index=False, chunksize=1000)
    ...
    if not changed.empty:
        ...
        for i, row in changed.iterrows():
            ...
            sql = f"UPDATE dbo.ce_plan SET is_active=0 WHERE id_pit={row['id_pit']} AND plan_date='{tgl_sql}' AND is_active=1"
            cursor.execute(sql)
        conn.commit()
        ...
        cols = ['id_pit', 'plan_date', 'plan_qty', 'version', 'is_active', 'data_provenance', 'created_at']
        changed[cols].to_sql('ce_plan', con=engine, if_exists='append', index=False, chunksize=1000)
...
with DAG(
    ...
    description='Ingest CE Data (Folder Isolated)',
